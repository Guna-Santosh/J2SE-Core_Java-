...
public class Pro8 {
	...
	public int sumofmultiples(int a, int b, int c) {
		...
		if (a <= 0 || b <= 0 || c <= 0)
			return -1;
		/*
		 * else if(a%2!=0) return (a*a*a); else if(b%2!=0) return (b*b*b); else
		 * if(c%2!=0) return(c*c*c);
		 */
		if (a % 10 == 0)
			a = (a / 10) * 10;
		else
		...
		return a + b + c;
		/*
		 * if(a%10!=0 && b%10!=0 && c%10!=0) { a=(a/10+1)*10; b=(b/10+1)*10;
		 * c=(c/10+1)*10; } else a=(a/10)*10; b=(b/10)*b; c=(c/10)*c; return a+b+c;
		 */
	}
